c===================================================================
c
c  VUMAT subroutine: 3D crystal plasticity of Cu-B4C composite
c
...
c 
c Remember to change NUM_EL and FILEPATH !!!
c===================================================================
c-------------------------------------------------------------------
...
c     maximum block size (maxblk)
       ...
       real*8    psi(num_el,3), F_p(3,3), phase(num_el)
       PI = 4.D0*ATAN(1.D0)
...
c     Read in Euler angles from external file and store in STATENEW           
         ...
         FILE1 = '/data/yinzhang/B4C_Cu/CPFEM/crystalline/aeuler'
         call VGETRANK(KPROCESSNUM)
         kunit = 65 + KPROCESSNUM
         open(unit=kunit,file=FILE1,action='READ',status='UNKNOWN')
         read(kunit,*)
         read(kunit,*)
         read(kunit,*) ICRYS_FLAG
         read(kunit,*)
         read(kunit,*)
         read(kunit,*)
         read(kunit,*)
         do i = 1,num_el
           read(kunit,*) ph, th, om, kphase 
           psi(i,1) = ph*PI/180.0
           psi(i,2) = th*PI/180.0
           psi(i,3) = om*PI/180.0
           phase(i) = kphase*1.D0
         ...
         do k = 1,nblock
           i = jElem(k)
c           if (KPROCESSNUM==0) write(*,*) i, psi(i,1), phase(i)
           stateNew(k,1) = psi(i,1)
           stateNew(k,2) = psi(i,2)
           stateNew(k,3) = psi(i,3)
           stateNew(k,84) = phase(i)
c           field(k,1) = 300.0
         end do
         return
       end if
...
C       
     ...
     &      num_el = 8000,          ! Total number of elements
     &      B_k = 1.38064852E-23)  ! Boltzmann constant in SI unit

       character*80 cmname
c-------------------------------------------------------------------
c  Dimension other arrays used in this VUMAT sub
c-------------------------------------------------------------------
       real*8
     & array1(3,3),
     & array2(3,3),
...
c====================================================================
       ...
       if(debug>0) write(*,*) "Material Properties"
...
C     same number of integration or material points) at a time. 
C
C====================================================================       
C==================================================================== 
C====================================================================       
...
c====================================================================
         ...
         if (totalTime.le.dt.or.dt.eq.one) then
           if(debug>0) write(*,*) "Initializing ISVs, first time step"
...
c-------------------- Initialize threshold stress --------------------
           ...
           if(debug>0) write(*,*) "Initialization finished!"
         end if
c====================================================================         
...
c====================================================================
         if (totalTime.gt.dt.and.dt.ne.one) then
           if(debug>0) write(*,*) "Read in ISVs, nonzero time step"
           n = 0
...
c-------------------- Read in PEEQ 82 ------------------------------
           if(debug>0) write(*,*) "ISVs read in finished!"
...
c========Determine phase index, 0 for matrix, 1 for secondary phase==
         if (stateOld(km,84) < 0.5) then
c====================================================================       
c       Define Dir_cos and Slip System
c====================================================================
           if(debug>0) write(*,*) "Define Dir_cos and Slip system!"
...
c======================================================================
           if(debug>0) write(*,*) "Define Kron and Elastic Tensor!"
           call def_Kron_Del(del)
           call calc_4th_C(C11, C12, C44, del, C0)
c======================================================================       
c        RRead in deformation gradient tensor
c======================================================================
           if(debug>0) write(*,*) "Read in deformation gradient tensor!"
...
c======================================================================
           if(debug>0) write(*,*) "Rotate Cijkl and slip system!"
...
c          (1.1) - (1.8)
c======================================================================
           if(debug>0) write(*,*) "Calculate the L_p, E_el, Spk2, Sig"
...
c======================================================================
           if(debug>0) write(*,*) "Updating rot matrix", F_p_0, F_p_t
...
c======================================================================
           ...
           if(debug>0) write(*,*) "Evolution of internal state variable"
...
c          dE = sig * de / density
c======================================================================
           if(debug>0)write(*,*) "Update the internal energy"
...
c======================================================================
           if(debug>0) write(*,*) "Store in state variables "
...
c---------------------- Store in phase index 84------------------------
           n = n + 1
           stateNew(km,n) = stateOld(km,n)
c-----------------------Store in elastic strain 85-87------------------
           n = n + 1
           stateNew(km,n) = E_el(1,1)
           n = n + 1
           stateNew(km,n) = E_el(2,2)
           n = n + 1
           stateNew(km,n) = E_el(3,3)

           if(debug>0) write(*,*) "Next Element!"
c          End of crystal plasticity


c====================================================================== 
c          Isotropic hardenging plasticity J2 for secondary phase
c          See P245 in "Writing User Subroutine with ABAQUS"        
c          Use PEEQ (SDV82) as ISVs
c======================================================================
         else
           twomu = E_sec / (1.D0 + anu_sec)
           alamda = anu_sec * twomu / (1.D0 - 2.D0*anu_sec)
           thremu = 1.5D0 * twomu
c          Radial return for 3D case
c          modify this part if u want plane stress/strain cases
           tr = strainInc(km,1) + strainInc(km,2) + strainInc(km,3)

           if(stepTime .eq. 0.D0) then
             s11 = stressOld(km,1)+twomu*strainInc(km,1)+alamda*tr
             s22 = stressOld(km,2)+twomu*strainInc(km,2)+alamda*tr
             s33 = stressOld(km,3)+twomu*strainInc(km,3)+alamda*tr
             s12 = stressOld(km,4)+twomu*strainInc(km,4)
             s23 = stressOld(km,5)+twomu*strainInc(km,5)
             s31 = stressOld(km,6)+twomu*strainInc(km,6) 
             stressNew(km,1) = s11
             stressNew(km,2) = s22
             stressNew(km,3) = s33
             stressNew(km,4) = s12
             ...
             yield = s0_sec + h0_sec*stateOld(km,82) 
             s11 = stressOld(km,1)+twomu*strainInc(km,1)+alamda*tr
             s22 = stressOld(km,2)+twomu*strainInc(km,2)+alamda*tr
             s33 = stressOld(km,3)+twomu*strainInc(km,3)+alamda*tr
             s12 = stressOld(km,4)+twomu*strainInc(km,4)
             s23 = stressOld(km,5)+twomu*strainInc(km,5)
             s31 = stressOld(km,6)+twomu*strainInc(km,6) 
             smean = (s11 + s22 + s33)/3.D0
             s11 = s11 - smean
             s22 = s22 - smean
             s33 = s33 - smean
             vmises = sqrt(1.5D0*(s11*s11 + s22*s22 + s33*s33
     &                + 2.D0*s12*s12 + 2.D0*s23*s23 + 2.D0*s31*s31))
             sigdif = vmises - yield
             facyld = 0.D0
             if (sigdif .gt. 0.D0) facyld = 1.D0
             deps = facyld * sigdif / (thremu + h0_sec)
c            Update stress
             yieldNew = yield + h0_sec * deps
             factor = yieldNew / (yieldNew + thremu * deps)
             stressNew(km,1) = s11 * factor + smean
             stressNew(km,2) = s22 * factor + smean
             stressNew(km,3) = s33 * factor + smean
             stressNew(km,4) = s12 * factor
             stressNew(km,5) = s23 * factor
             stressNew(km,6) = s31 * factor
c            Update plastic strain
             stateNew(km,46) = yieldNew
             stateNew(km,82) = stateOld(km,82) + deps
             stateNew(km,84) = stateOld(km,84)
           end if
c          Update specific internal energy
           enerInternNew(km) = enerInternOld(km)
           do i = 1, ndir+nshr
             enerInternNew(km) = enerInternNew(km) + 
     &       0.5D0*(stressOld(km,i)+stressNew(km,i))
     &       *strainInc(km,i)/density(km)
             if(i.gt.ndir) then
               enerInternNew(km) = enerInternNew(km) + 
     &         0.5D0*(stressOld(km,i)+stressNew(km,i))
     &         *strainInc(km,i)/density(km)
           ...
           plastic_work_inc = 0.5D0*(yield + yieldNew)*deps
           enerInelasNew(km) = enerInelasOld(km) + 
     &                         plastic_work_inc/density(km)
 
         end if
c        End of J2 plasticity       

       end do 
c      End of for loop of nblock

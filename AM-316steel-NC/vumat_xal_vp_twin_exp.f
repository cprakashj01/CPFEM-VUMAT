...
C      TEXCALTIME= props(11) -- Set equal to the time at which you wish to
C                               first calculate the texture. 
C      TEXFREQ   = props(12) -- Set equal to the time increment 
...
C      S0_twin   = props(13) -- Ininital val. of def. resis., Pa for
C									nanotwinned slip systems
C      SSAT_twin = props(14) -- Sat. val. for twinned slip systems
C      AH0_twin  = props(15) -- Value of the initial hardening rate, Pa
C	   RHARD_twin = props(16) -- Exponent in hardening equation
C
C            These properties are specified in *.inp file
C      *USER MATERIAL,CONSTANTS=16
C
C      in the main input deck.
C
C
C      The model also use 18 solution dependent variables 
...
C		SDV10--SDV15	   --- Backstress component
...
C
C	    PARAMETER(NOEL=8000,NCRYS=1,NSLIP=12)
C
C 2.      aeuler:
C
...
C         in each crystal.
C****************************************************************
C     INCLUDE '/u1/zhut/backup/Cu_09/Abaqus/crystal/vumatcomm'
      PARAMETER(NOEL=8000,NCRYS=1,NSLIP=12,screw=1.)
     ...
     +        ICRYS_FLAG,IEULERERR 

C     REAL*8 GDOT0,AM,S0,AH0,SSAT,RHARD
C     REAL*8 QL,TEXCALTIME,TEXFREQ
...
C**********************************************************************
      	COMMON/VUMATPRP/C11, C12,C44,GDOT0,AM,S0,AH0,SSAT,RHARD,QL,
     +        TEXCALTIME,TEXFREQ,S0_Twin,SSAT_Twin,AH0_Twin,RHARD_Twin                 
...
C           endif       
      ...
      FILE1 = '~/Polycrystalline/Residual_stress/26/aeuler'
      FILE2 = '~/Polycrystalline/Residual_stress/slipsys'
      FILE3 = '~/Polycrystalline/Residual_stress/26/VU_MESSAGES'
      FILE4 = '~/Polycrystalline/Residual_stress/26/VU_TEXTURES'
      FILE5 = '~/Polycrystalline/Residual_stress/26/VU_SLIPS'
...
C      Back Stress      
       STATE_OLD(KM,10) = -30E6 
       STATE_OLD(KM,11) = -30E6
       STATE_OLD(KM,12) = 60E6
...
C
	   ...
	   DO 10 ICRYS = 1,NCRYS
C             pause
	        READ(65,*) PH, TH, OM, EIG11, EIG22, EIG33
C               write(*,*) TH, PH, OM
C              pause
		    VOL(ICRYS) = ONE
C		
C		    READ(65,*) PSIK, THK, PHIK, VOL(ICRYS)
C	        TH = THK
C		    PH = 90. + PHIK
C		    OM = 90. - PSIK
C
...
C
	      ...
	      DO 11 ICRYS = 1,NCRYS
	        ...
	        Q_ALL(INOEL,ICRYS,1,3) = Q_CRYS(ICRYS,1,3)
...
C
C        FILE2 = '/usr/home/cricket/tzhu/SXAL_VUMAT/FCC12'
C           pause
...
C
	    DO 45 ISLIP = 1,NSLIP
	      SMAT_C(ISLIP,1,1) = AMV_C(ISLIP,1)*ANV_C(ISLIP,1)
	      SMAT_C(ISLIP,1,2) = AMV_C(ISLIP,1)*ANV_C(ISLIP,2)
	      SMAT_C(ISLIP,1,3) = AMV_C(ISLIP,1)*ANV_C(ISLIP,3)
	      SMAT_C(ISLIP,2,1) = AMV_C(ISLIP,2)*ANV_C(ISLIP,1)
	      SMAT_C(ISLIP,2,2) = AMV_C(ISLIP,2)*ANV_C(ISLIP,2)
	      SMAT_C(ISLIP,2,3) = AMV_C(ISLIP,2)*ANV_C(ISLIP,3)
	      SMAT_C(ISLIP,3,1) = AMV_C(ISLIP,3)*ANV_C(ISLIP,1)
	      SMAT_C(ISLIP,3,2) = AMV_C(ISLIP,3)*ANV_C(ISLIP,2)
	      SMAT_C(ISLIP,3,3) = AMV_C(ISLIP,3)*ANV_C(ISLIP,3)
45	    CONTINUE
C
C            Calculate the components of the Schmid tensors in the crystal
...
C              
	     DO 55 ISLIP = 1,NSLIP
               if(islip.le.3) then
               S_T_ALL(INOEL,ICRYS,ISLIP)       = S0_Twin
               else
                if(islip.eq.6.or.islip.eq.9.or.islip.eq.12) then
	         S_T_ALL(INOEL,ICRYS,ISLIP)     = S0*screw
                else
                  if(islip.eq.15.or.islip.eq.18.or.islip.eq.21) then
                   S_T_ALL(INOEL,ICRYS,ISLIP)     = S0*screw
                  else
...
C
     ...
     +       DET_FSTAR_TAU_INV
        ...
        IF(TOTAL_TIME.GE.10) THEN
         Interval = 20.0
         IF(STEP_TIME.LE.30) THEN
            Interval = 1.0
         END IF
         IF(MOD(STEP_TIME,Interval).LE.DTIME) THEN
          WRITE(100,152) ESTAR_TAU(1,1), ESTAR_TAU(2,2), ESTAR_TAU(3,3),
     +    ESTAR_TAU(1,2), ESTAR_TAU(1,3), ESTAR_TAU(2,3), T_TAU(2,2),
     +    STATE_OLD(KM,1)
152       FORMAT(' ',8E12.4)
         END IF
        END IF
...
C                  write(*,*) 'SA_T',SA_T
                   ...
                   if(ISLIP.LE.3) Then
                   SFRAC = ONE - SA_T/SSAT_twin
                   else
                    if(islip.eq.6.or.islip.eq.9.or.islip.eq.12) then
                      SFRAC = ONE - SA_T/(SSAT*screw)
                     else
                      if(islip.eq.15.or.islip.eq.18.or.islip.eq.21) then
                       SFRAC = ONE - SA_T/(SSAT*screw)
                       else
 	               SFRAC = ONE - SA_T/(SSAT*1)
                       endif
                    endif
                   endif
 	         ...
 	         IF (SFRAC .GE. ZERO) THEN
                   if(ISLIP.LE.3) Then
	            AH_T(ISLIP) = AH0_Twin*(DABS(SFRAC)**RHARD_Twin)
                   else
...
C              and the accumulated sshears for all the slip systems
C
	    ...
	    DO 140 ISLIP = 1,NSLIP
          ...
          Bij(1,1) = Bij(1,1) +
     +    C44*EXP(-3000*DABS(ACCGAM_TAU(ISLIP)))*DGAMMA_TAU(ISLIP)*
...
170	  CONTINUE
	  ...
	  IF (DTIME .NE. ONE) THEN
	    ...
	    STATE_NEW(KM,3) = EQSTRESS_TAU
            ...
            STATE_NEW(KM,16) = RSS_TAU(13)
            STATE_NEW(KM,17) = RSS_TAU(14)
            STATE_NEW(KM,18) = RSS_TAU(15)

C           write(*,*) 'es',EQSTRESS_TAU
	  ENDIF
C	  
...
C           if (NOEL_PT+KM.eq.1) then
C           write(*,*) 'write', DGAMMA_T_ALL(1,1,4)
C           endif      
